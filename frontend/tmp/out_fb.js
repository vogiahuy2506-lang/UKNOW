...
export default function FacebookSettings() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || err.message || "Kh\xF4ng th\u1EC3 kh\u1EDFi t\u1EA1o OAuth.";
      toast.error(msg);
    } finally {
      setConnecting(false);
    }
  ...
  const handleRefresh = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || err.message || "L\xE0m m\u1EDBi token th\u1EA5t b\u1EA1i.");
    } finally {
