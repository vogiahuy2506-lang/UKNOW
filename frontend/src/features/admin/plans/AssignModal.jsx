    ...
    <form onSubmit={handleAssign} className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Gói: <strong>{plan.name}</strong> ·{' '}
          {billingPeriod === 'yearly' && plan.priceYearly
            ? `${fmtVnd(plan.priceYearly)}/năm`
            : billingPeriod === 'yearly'
              ? `${fmtVnd(plan.price * 12)}/năm (chưa cấu hình giá năm riêng)`
              : `${fmtVnd(plan.price)}/tháng`}
        </p>
      </div>
      <div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button type="submit" className="btn btn-primary" disabled={isLoading || quantity === ''}>
