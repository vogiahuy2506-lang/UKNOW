 ...
 * lưu lại (backend tự strip iframe khi lưu). Bản gốc 50c05cd2 còn render occupation/interestArea/
 * customFields động (`config.fixedFields`/`config.customFields`) + theme tuỳ biến qua
 * `normalizeLeadFormTheme` — nhưng `landingLeadFormConfig.js` hiện tại (3c514bc8) không còn
 * export các field/hàm đó (không còn UI admin cấu hình). Bản này CHỦ ĐỘNG bỏ occupation/
 * interestArea/customFields/theme động, chỉ giữ name/email/phone/marketingConsent tĩnh — đúng
 * những gì landingLeadFormConfig.js hiện còn cung cấp; không còn nhận prop `leadFormConfig`/`theme`.
 */
 ...
 */
export function FounderLeadFormCard({
  formCopy,
  form,
  setField,
  submitting,
  ...
  const fieldSurface = isEmbed
    ...
    : 'border-founder-border bg-founder-cream focus:border-founder-teal focus:bg-white';
      ...
      <div
        className={
          isEmbed
            ...
            : 'relative z-[2] w-full max-w-[430px] rounded-[20px] border border-founder-border bg-white px-[34px] pb-10 pt-10 shadow-[0_24px_64px_rgba(11,85,99,0.1),0_4px_16px_rgba(0,0,0,0.04)]'
        }
      >
        {!isEmbed ? (
          <div
        ...
        <div className={isEmbed ? 'py-2 text-center' : 'py-5 text-center'}>
          ...
          >
            <svg
              className={isEmbed ? 'h-6 w-6 text-founder-teal' : 'h-7 w-7 text-founder-teal'}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="2.5"
          ...
          <h3 className={isEmbed ? 'text-lg font-bold text-gray-900' : 'text-xl font-extrabold text-founder-ink'}>
          ...
          <p
            className={
              isEmbed
                ...
                : 'mt-3 text-[0.88rem] leading-relaxed text-founder-muted'
            }
          >
  ...
  return (
    ...
    >
      ...
      {isEmbed ? (
        ...
        <div className="mb-6">
          <p className="mb-2 flex items-center gap-1.5 text-[0.7rem] font-bold uppercase tracking-[1.5px] text-founder-teal">
            <span aria-hidden>🎁</span> {formCopy.cardEyebrow}
          </p>
          <h2 className="font-landing text-[1.4rem] font-black leading-tight tracking-[-0.3px] text-founder-ink">
            {formCopy.cardTitleLine1}
            <br />
            {formCopy.cardTitleLine2}
          </h2>
          <p className="mt-2 text-[0.84rem] leading-relaxed text-founder-muted">{formCopy.cardSubtitle}</p>
        </div>
      )}
      ...
      >
        ...
        <div>
          <label htmlFor="founder-phone" className="mb-1.5 block text-[0.78rem] font-semibold text-[#3a3a3a]">
        ...
        <div className="flex items-start gap-2.5 pt-1">
          <input
            ...
            className="mt-0.5 h-4 w-4 shrink-0 cursor-pointer accent-founder-teal"
          />
          <label htmlFor="founder-agree" className="cursor-pointer text-[0.78rem] leading-relaxed text-founder-muted">
            {formCopy.consentPrefix}{' '}
            <a
              href="/privacy-policy"
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-founder-teal underline underline-offset-2"
            >
        ...
        <div
          className={
            isEmbed
              ...
              : 'flex items-center justify-center gap-1.5 pt-1 text-[0.72rem] text-founder-muted'
          }
        >
