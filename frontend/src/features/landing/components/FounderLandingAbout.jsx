 ...
 */
export function FounderLandingAbout({ about }) {
  if (!about) return null;

  return (
    <section className="relative overflow-hidden bg-founder-ink px-[6%] py-24 text-white sm:px-[8%] sm:py-[100px]">
      <div
      ...
      <div className="relative z-[2] mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-20">
        ...
        <div className="relative mx-auto w-full max-w-[440px] lg:mx-0">
          <div className="relative">
            <div className="absolute -inset-4 bottom-0 left-4 right-[-16px] top-[-16px] rounded-[20px] bg-gradient-to-br from-founder-teal to-founder-gold opacity-25" />
            <div className="relative z-[1] aspect-[3/4] overflow-hidden rounded-2xl border-2 border-white/10 bg-founder-ink">
              <img
              ...
              <div className="absolute bottom-0 left-0 right-0 z-[2] px-4 pb-5 pt-12 text-center">
                ...
                <div className="mt-1 text-[0.78rem] text-founder-gold-light">{about.photoDegree}</div>
              </div>
            </div>

            <div className="animate-founder-float-1 absolute left-0 top-[10%] z-[3] flex max-w-[200px] items-center gap-2 rounded-[10px] bg-white px-3.5 py-2.5 text-[0.78rem] font-semibold text-founder-ink shadow-[0_8px_24px_rgba(0,0,0,0.3)] sm:left-[-20px]">
            ...
            <div className="animate-founder-float-2 absolute bottom-[15%] right-0 z-[3] flex max-w-[200px] items-center gap-2 rounded-[10px] bg-white px-3.5 py-2.5 text-[0.78rem] font-semibold text-founder-ink shadow-[0_8px_24px_rgba(0,0,0,0.3)] sm:right-[-20px]">
              <span className="text-xl" aria-hidden>
                🎓
              </span>
              <span className="leading-snug">
        ...
        <div>
          <p className="mb-3.5 text-[0.68rem] font-bold uppercase tracking-[2px] text-founder-gold-light">{about.label}</p>
          <h2 className="font-landing text-[clamp(2rem,3.5vw,3rem)] font-black leading-tight tracking-[-1px]">{about.name}</h2>
          <p className="mt-2 text-[0.9rem] font-medium text-founder-gold-light">{about.degree}</p>
          ...
          <div className="mt-8 grid grid-cols-3 gap-4 border-t border-white/10 pt-7">
            {about.stats.map((s) => (
              <div key={s.label}>
                <div className="font-landing text-[2rem] font-black leading-none text-founder-gold-light">{s.value}</div>
