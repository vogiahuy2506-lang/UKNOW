...
const ResetPasswordPage = () => {
  ...
  if (!token) {
    return (
      <div className="w-full text-center space-y-4 py-8">
        ...
        <h2 className="text-xl font-bold text-gray-900">{t('resetPassword.invalidLink')}</h2>
        <p className="text-gray-500 text-sm">{t('resetPassword.pleaseRequestAgain')}</p>
        <Link to="/forgot-password" className="btn btn-primary block w-full">{t('resetPassword.requestReset')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      <div className="mb-10">
        <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight mb-2">{t('resetPassword.title')}</h1>
        <p className="text-slate-500 font-medium">{t('resetPassword.subtitle')}</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="space-y-1.5">
          <label className="block text-sm font-bold text-slate-700">{t('resetPassword.newPasswordLabel')}</label>
          <input
            type="password"
        ...
        <div className="space-y-1.5">
          <label className="block text-sm font-bold text-slate-700">{t('resetPassword.confirmPasswordLabel')}</label>
          <input
            type="password"
            className={`w-full px-4 py-3.5 border rounded-xl outline-none transition-all duration-200 bg-slate-50 focus:bg-white ${
        ...
        >
          {isSubmitting ? t('resetPassword.saving') : t('resetPassword.submitButton')}
        </button>
      </form>
    </div>
  );
