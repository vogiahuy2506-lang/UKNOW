...
CREATE INDEX idx_login_history_user ON login_history(id_user);
CREATE INDEX idx_login_history_email_created ON login_history(email, created_at DESC);
...
CREATE UNIQUE INDEX idx_courses_unique_workspace_code ON courses((COALESCE(workspace_owner_id, id_user)), course_code) WHERE course_code IS NOT NULL;
...
-- ─── Chatbot contact alerts (migration 219) ──────────────────────────────────
...
-- ─── Migration 178: Wave 2 send_quota_reservations ───────────────────────────
...
ALTER TABLE email_messages
  ADD COLUMN IF NOT EXISTS quota_reservation_id BIGINT;
...
ALTER TABLE zalo_personal_messages
  ADD COLUMN IF NOT EXISTS quota_reservation_id BIGINT;
...
-- ─── Forms & Form Submissions (migration 216_forms_and_submissions.sql) ───────
...
CREATE TABLE IF NOT EXISTS form_submissions (
  ...
  payment_receipt_waived_reason VARCHAR(40),
  payment_receipt_upload_count SMALLINT NOT NULL DEFAULT 0,
...
-- --- Migration 228: notification_templates (super admin save-as) ------------
