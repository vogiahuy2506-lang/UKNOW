...
class CustomDomainController {
  ...
  /**
   * POST /api/custom-domains/:id/setup-cloudflare
   * Setup domain with Cloudflare (DNS + SSL).
   */
  async setupCloudflare(req, res) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      const role = req.user?.role;

      const result = await customDomainService.setupWithCloudflare(
        Number.parseInt(id, 10),
        { userId, role }
      );
      ...
      console.error('[CustomDomain] Cloudflare setup error:', error);
      res.status(500).json({
        success: false,
        message: error.message,
      });
  ...
  /**
   * GET /api/custom-domains/:id/cloudflare-status
  ...
  async getCloudflareStatus(req, res) {
    try {
      ...
      const status = await customDomainService.getCloudflareStatus(
        Number.parseInt(id, 10),
        { userId, role }
      );
      ...
      console.error('[CustomDomain] Cloudflare status error:', error);
      res.status(500).json({
        success: false,
        message: error.message,
      });
  ...
  /**
   * POST /api/custom-domains/:id/verify-cloudflare
   * Verify domain using Cloudflare API.
   */
  async verifyWithCloudflare(req, res) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      const role = req.user?.role;

      const result = await customDomainService.verifyDomainWithCloudflare(
        Number.parseInt(id, 10),
        { userId, role }
      );
      ...
      console.error('[CustomDomain] Cloudflare verify error:', error);
      res.status(500).json({
        success: false,
        message: error.message,
      });
