...
function employeeHasPermission(req, permission) {
  ...
  return context?.type !== 'employee' || context.permissions?.[permission] === true;
}

function denyCampaignRun(res) {
...
class AiController {
  ...
  async executeCampaign(req, res) {
    try {
      // Authorize auto-run before campaignController.create mutates the workspace.
      if (req.body.autoRun && !employeeHasPermission(req, 'campaigns_run')) {
      ...
      if (req.body.autoRun && createRes.data?.success) {
        ...
        const runReq = {
          ...req,
          campaignVia: resolveCampaignVia({}),
  ...
  async createCampaignFromDraft(req, res) {
    ...
    try {
      ...
      const createReq = {
        ...
        campaignVia: resolveCampaignVia(preparedScript),
        body: {
  ...
  async pushToCampaign(req, res) {
    try {
      ...
      // Authorize auto-run before campaignController.update mutates the campaign.
      if (autoRun && !employeeHasPermission(req, 'campaigns_run')) {
        return denyCampaignRun(res);
      }
  ...
  async createAndRunCampaign(req, res) {
    ...
    try {
      ...
      if (!employeeHasPermission(req, 'campaigns_create') || !employeeHasPermission(req, 'campaigns_run')) {
        return res.status(403).json({
          success: false,
          message: 'Bạn cần quyền tạo và chạy chiến dịch để thực hiện thao tác này',
          code: 'PERMISSION_DENIED',
        });
      }
