...
/**
 * @typedef {Object} PlanChangeResolution
 * @property {'upgrade_now' | 'schedule' | 'upgrade_pending' | 'blocked'} action
 * @property {number} amountToPay
 * @property {string|null} code - Error code when blocked (SAME_PLAN, YEARLY_TO_MONTHLY, PENDING_DOWNGRADE)
 * @property {string|null} message - User-friendly message
 * @property {Date|string|null} activateAfter - Target activation date if scheduled
 * @property {number|null} pendingChangeId - If replacing an existing pending change
 * @property {number} daysRemaining - Days left in current active plan
 */

/**
 * Resolve the appropriate action and billing amount when changing plans.
 *
 * @param {Object} params
 * @param {Object|null} params.currentPlan - Current user's plan object { id, price, price_yearly, ... }
 * @param {'monthly'|'yearly'|null} params.currentBillingPeriod
 * @param {Date|string|null} params.subscriptionExpiresAt
 * @param {Object} params.targetPlan - Target plan object { id, price, price_yearly, ... }
 * @param {'monthly'|'yearly'} params.targetBillingPeriod
 * @param {Object|null} params.pendingChange - Active pending scheduled change { id, plan_id, billing_period, amount_paid, activate_after, plan_price, plan_price_yearly }
 * @param {number} [params.targetPlanPrice] - Overridden final price for target plan (e.g. after custom plan quote)
 * @param {Date} [params.now]
 * @returns {PlanChangeResolution}
 */
