...
class UknowReadService {
  /**
   * Proxy Founder AI customers list API.
   *
  ...
  async getCustomers(ctx, req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Khong the lay du lieu khach hang tu Founder AI',
      });
    }
  }

  /**
   * Proxy Founder AI products list API.
   *
   * @param {object} ctx controller context
   * @param {import('express').Request} req
   * @param {import('express').Response} res
  ...
  async getCourses(ctx, req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Khong the lay du lieu khoa hoc tu Founder AI',
      });
    }
  }

  /**
   * Proxy Founder AI orders list API.
   *
   * @param {object} ctx controller context
   * @param {import('express').Request} req
   * @param {import('express').Response} res
  ...
  async getOrders(ctx, req, res) {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Khong the lay du lieu don hang tu Founder AI',
      });
    }
  }

  /**
   * Proxy Founder AI single order API.
   *
   * @param {object} ctx controller context
   * @param {import('express').Request} req
   * @param {import('express').Response} res
  ...
  async getOrder(ctx, req, res) {
    try {
      ...
      res.status(500).json({ success: false, message: 'Không thể lấy thông tin đơn hàng từ Founder AI' });
    }
  }
}
